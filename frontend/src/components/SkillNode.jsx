...
const SkillNode = ({ node, status, isActive, onClick, isReadOnly }) => {
  ...
  return (
    <button
      ...
      className={`quest-node relative w-full overflow-hidden rounded-2xl border-2 p-4 text-left backdrop-blur transition-all duration-300 ${STATUS_STYLE[status]} ${
        (!isUnlocked || isReadOnly) ? (isReadOnly ? "cursor-default opacity-90" : "cursor-not-allowed opacity-85") : "cursor-pointer"
