    ...
    try {
      ...
      const payload =
        mode === "login"
          ...
          : { username: form.username, password: form.password, school_id: Number(form.school_id) };
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-xl items-center px-4 py-10">
      <form onSubmit={submit} className="w-full rounded-3xl bg-white/85 p-6 shadow-soft">
        <h1 className="font-['Nunito'] text-3xl font-extrabold">{mode === "login" ? "Student Login" : "Student Signup"}</h1>
        ...
        <button disabled={loading} className="mt-4 w-full rounded-xl bg-brand-ocean px-4 py-3 font-bold text-white">
        ...
        >
          {mode === "login" ? "New student? Signup" : "Already have account? Login"}
        </button>
      </form>
    </div>
  );
